...
{
    VRAM_TRACE_PRINT("VRAM_GET_BYTE: address: 0x%02x, value: 0x%02x\n", address, self->vram_byte[address]);
...
void vram_set_byte(struct Vram *self, uint16_t address, uint8_t byte)
{
    ...
    VRAM_TRACE_PRINT("VRAM_SET_BYTE: address: 0x%02x, value: 0x%02x\n", address, byte);
}
...
uint16_t vram_get_word(struct Vram *self, uint16_t address)
{
    ...
    VRAM_TRACE_PRINT("VRAM_GET_WORD: address: 0x%02x, value: 0x%04x\n", address, word);
    return word;
}
...
void vram_set_word(struct Vram *self, uint16_t address, uint16_t word)
{
    ...
    VRAM_TRACE_PRINT("VRAM_SET_WORD: address: 0x%02x, value: 0x%04x\n", address, word);
}
